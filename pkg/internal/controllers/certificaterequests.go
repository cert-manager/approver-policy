...
func (c *certificaterequests) reconcileStatusPatch(ctx context.Context, req ctrl.Request) (ctrl.Result, *cmapi.CertificateRequestStatus, error) {
	...
	case manager.ResultApproved:
		...
		c.recorder.Eventf(cr, corev1.EventTypeNormal, "Approved", "%s", response.Message)
	...
	case manager.ResultDenied:
		...
		c.recorder.Eventf(cr, corev1.EventTypeWarning, "Denied", "%s", response.Message)
	...
	case manager.ResultUnprocessed:
		...
		c.recorder.Eventf(cr, corev1.EventTypeNormal, "Unprocessed", "Request is not applicable for any policy so ignoring")

		return ctrl.Result{}, nil, nil

	default:
		log.Error(errors.New(response.Message), "manager responded with an unknown result", "result", response.Result)
		c.recorder.Eventf(cr, corev1.EventTypeWarning, "UnknownResponse", "Policy returned an unknown result. This is a bug. Please check the approver-policy logs and file an issue")
